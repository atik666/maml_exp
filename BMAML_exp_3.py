...
indx_neg = rep_image(image_array_cluster, class_array_cluster)

img_pos_array = [cv2.imread(path+indx[i]) for i in range(len(indx))]

img_neg_array = [cv2.imread(path+indx_neg[i]) for i in range(len(indx_neg))]


similarity = [orb_sim(img_pos_array[0], img_neg_array[i]) for i in range(len(image_array_cluster))]
index_min = np.argmin(similarity)
selected_cluster = class_array_cluster[index_min]
